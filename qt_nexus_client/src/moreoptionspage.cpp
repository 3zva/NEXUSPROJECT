...
MoreOptionsPage::MoreOptionsPage(QWidget* parent)
    ...
    connect(m_monitoringToggle, &ToggleRow::toggled, this, [this](bool enabled) {
        ...
        Q_EMIT runtimeHelperMonitoringEnabledChanged(enabled);
        Q_EMIT settingChanged(QStringLiteral("runtime_helper/enabled"), enabled);
    });
    connect(cursorToggle, &ToggleRow::toggled, this, [this](bool enabled) {
        Q_EMIT settingChanged(QStringLiteral("runtime_helper/cursor_visible_only"), enabled);
    });
    connect(activeWindowToggle, &ToggleRow::toggled, this, [this](bool enabled) {
        Q_EMIT settingChanged(QStringLiteral("runtime_helper/target_window_active_only"), enabled);
    });
    ...
    handoffLayout->addWidget(createSectionLabel(
        QStringLiteral("RUNTIME HELPER INTEGRATION STATUS"),
        handoff
    ));
    handoffLayout->addWidget(mutedText(
        QStringLiteral(
            "The page is fully routed and exposes typed Qt signals. Connect "
            "regionSelectionRequested() to the region-selection runtime helper you are already building, "
