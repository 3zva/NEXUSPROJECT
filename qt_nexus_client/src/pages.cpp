...
ClientSettingsPage::ClientSettingsPage(QWidget* parent)
    ...
    auto* detector = new CardFrame(settings, false);
    auto* detectorLayout = new QVBoxLayout(detector);
    detectorLayout->setContentsMargins(16, 12, 16, 12);
    detectorLayout->setSpacing(10);

    auto* detectorHeader = new QHBoxLayout();
    auto* detectorText = new QVBoxLayout();
    detectorText->setSpacing(2);
    auto* detectorLabel = new QLabel(QStringLiteral("Native detector"), detector);
    detectorLabel->setFont(NexusTheme::font(10, QFont::DemiBold));
    auto* detectorDescription = new QLabel(QStringLiteral("Run the bundled body/head detector helper with saved input settings."), detector);
    detectorDescription->setProperty("muted", true);
    detectorDescription->setWordWrap(true);
    detectorText->addWidget(detectorLabel);
    detectorText->addWidget(detectorDescription);
    ...
    auto createDetectorToggle = [this, detector, detectorLayout, &storedSettings](
        const QString& label,
        const QString& key,
        bool defaultValue
    ) {
    ...
    createDetectorToggle(QStringLiteral("Show preview window?"), QStringLiteral("native_detector/preview"), false);
    createDetectorToggle(QStringLiteral("Show status window?"), QStringLiteral("native_detector/status_window"), true);
    createDetectorToggle(QStringLiteral("Enable trigger logic?"), QStringLiteral("native_detector/trigger_enabled"), true);
    createDetectorToggle(QStringLiteral("Enable automated LMB?"), QStringLiteral("native_detector/lmb_enabled"), true);
    createDetectorToggle(QStringLiteral("Use hold-B activation mode?"), QStringLiteral("native_detector/b_hold_mode_enabled"), false);

    auto createDetectorSlider = [this, detector, detectorLayout, &storedSettings](
        const QString& label,
        const QString& key,
        int minimum,
        int maximum,
    ...
    createDetectorSlider(QStringLiteral("Confidence"), QStringLiteral("native_detector/confidence_percent"), 5, 95, 30, QStringLiteral("%"));
    createDetectorSlider(QStringLiteral("FPS cap"), QStringLiteral("native_detector/fps_cap"), 0, 240, 0, QString());
    createDetectorSlider(QStringLiteral("Hold delay"), QStringLiteral("native_detector/hold_delay_ms"), 0, 500, 185, QStringLiteral("ms"));
    createDetectorSlider(QStringLiteral("Press delay"), QStringLiteral("native_detector/trigger_press_delay_ms"), 0, 800, 392, QStringLiteral("ms"));
    createDetectorSlider(QStringLiteral("Gate width"), QStringLiteral("native_detector/activation_gate_width"), 20, 420, 120, QString());
    createDetectorSlider(QStringLiteral("Gate height"), QStringLiteral("native_detector/activation_gate_height"), 20, 420, 120, QString());
    ...
    settingsLayout->addWidget(detector);
