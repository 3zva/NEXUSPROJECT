...
constexpr wchar_t DEFAULT_CONFIG_FILENAME[] = L"nexus_runtime_helper_config.txt";
...
void Log(const std::string& message) {
    std::ofstream out("nexus_runtime_helper.log", std::ios::app);
    out << message << "\n";
}
...
RuntimeHelperConfig LoadConfig(const fs::path& path) {
    RuntimeHelperConfig config;
...
bool HasUsableRegion(const RuntimeHelperConfig& config) {
...
class OcrEngine {
public:
    explicit OcrEngine(const RuntimeHelperConfig& config) {
...
bool PostOperatorDetectionToPort(int port, const std::string& operatorName, int confidence) {
    HINTERNET session = WinHttpOpen(L"NexusRuntimeHelper/1.0", WINHTTP_ACCESS_TYPE_NO_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    if (!session) {
...
int RunRuntimeHelper(const RuntimeHelperConfig& config) {
...
RuntimeHelperConfig WaitForConfiguredRegion(const fs::path& configPath) {
    auto nextLog = std::chrono::steady_clock::time_point{};
    while (g_running) {
        RuntimeHelperConfig config = LoadConfig(configPath);
        if (HasUsableRegion(config)) {
            Log(
                "NEXUS runtime helper loaded saved region: x=" + std::to_string(config.region.x)
        ...
        if (now >= nextLog) {
            Log("NEXUS runtime helper idle: waiting for region_configured=true in " + configPath.string());
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
    ...
    try {
        ...
        g_singleInstanceMutex = CreateMutexW(nullptr, TRUE, L"Global\\NexusRuntimeHelperSingleton");
        if (g_singleInstanceMutex != nullptr && GetLastError() == ERROR_ALREADY_EXISTS) {
            Log("NEXUS runtime helper exited: another monitor instance is already running.");
            if (argv) LocalFree(argv);
        ...
        if (argv) LocalFree(argv);
        RuntimeHelperConfig config = WaitForConfiguredRegion(configPath);
        if (!HasUsableRegion(config)) return 0;
        Log("NEXUS runtime helper started with " + std::to_string(config.operators.size()) + " operators.");
        const int result = RunRuntimeHelper(config);
